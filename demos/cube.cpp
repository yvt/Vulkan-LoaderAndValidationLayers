...
struct Demo
{
    ...
    vk::Bool32 demo_check_layers(uint32_t check_count, char const*const*const check_names, uint32_t layer_count,
    ...
    void demo_cleanup()
    {
        ...
        device.destroySemaphore(image_acquired_semaphore, nullptr);
        device.destroySemaphore(draw_complete_semaphore, nullptr);
    ...
    void demo_destroy_texture_image(texture_object *tex_objs)
    {
    ...
    void demo_draw()
    {
        ...
        auto result = device.acquireNextImageKHR(swapchain, UINT64_MAX, image_acquired_semaphore, vk::Fence(),
            &current_buffer);
        ...
        auto const submitInfo = vk::SubmitInfo()
            ...
            .setPSignalSemaphores(&draw_complete_semaphore);

        result = graphics_queue.submit(1, &submitInfo, vk::Fence());
        VERIFY(result == vk::Result::eSuccess);

        auto const presentInfo = vk::PresentInfoKHR()
            .setWaitSemaphoreCount(1)
            .setPWaitSemaphores(&draw_complete_semaphore)
            .setSwapchainCount(1)
    ...
    void demo_draw_build_cmd(vk::CommandBuffer commandBuffer)
    {
    ...
    void demo_init_vk()
    {
        ...
        if(validate)
        {
            ...
            {
                vk::LayerProperties *instance_layers = (vk::LayerProperties*) malloc(sizeof (vk::LayerProperties) * instance_layer_count);
                result = vk::enumerateInstanceLayerProperties(&instance_layer_count, instance_layers);
                VERIFY(result == vk::Result::eSuccess);
        ...
        if(gpu_count > 0)
        {
            vk::PhysicalDevice *physical_devices = (vk::PhysicalDevice*) malloc(sizeof(vk::PhysicalDevice) * gpu_count);
            result = inst.enumeratePhysicalDevices(&gpu_count, physical_devices);
            VERIFY(result == vk::Result::eSuccess);
        ...
        if(device_extension_count > 0)
        {
            ...
            result = gpu.enumerateDeviceExtensionProperties(nullptr, &device_extension_count, device_extensions);
            VERIFY(result == vk::Result::eSuccess);
        ...
        queue_props = (vk::QueueFamilyProperties *)malloc(queue_count * sizeof(vk::QueueFamilyProperties));
        gpu.getQueueFamilyProperties(&queue_count, queue_props);

        // Find a queue that supports gfx
        for(uint32_t gfx_queue_idx = 0; gfx_queue_idx < queue_count; ++gfx_queue_idx)
        {
            if(queue_props[gfx_queue_idx].queueFlags & vk::QueueFlagBits::eGraphics)
            {
                break;
            }
        }
    ...
    // Create a WSI surface for the window:
        ...
        vk::Bool32* supportsPresent = (vk::Bool32 *)malloc(queue_count * sizeof(vk::Bool32));
        ...
        for(uint32_t i = 0; i < queue_count; i++)
        {
            if((queue_props[i].queueFlags & vk::QueueFlagBits::eGraphics) && graphicsQueueNodeIndex == UINT32_MAX)
            {
        ...
        present_queue_family_index = presentQueueNodeIndex;
        ...
        vk::SurfaceFormatKHR *surfFormats = (vk::SurfaceFormatKHR *)malloc(formatCount * sizeof(vk::SurfaceFormatKHR));
        result = gpu.getSurfaceFormatsKHR(surface, &formatCount, surfFormats);
        VERIFY(result == vk::Result::eSuccess);
        ...
        result = device.createSemaphore(&semaphoreCreateInfo, nullptr, &image_acquired_semaphore);
        VERIFY(result == vk::Result::eSuccess);

        result = device.createSemaphore(&semaphoreCreateInfo, nullptr, &draw_complete_semaphore);
        VERIFY(result == vk::Result::eSuccess);
    ...
    void demo_prepare()
    {
        ...
        {
            current_buffer = i;
            demo_draw_build_cmd(buffers[i].cmd);
        }
    ...
    void demo_prepare_buffers()
    {
        ...
        vk::PresentModeKHR *presentModes = (vk::PresentModeKHR*)malloc(presentModeCount * sizeof(vk::PresentModeKHR));
        result = gpu.getSurfacePresentModesKHR(surface, &presentModeCount, presentModes);
        VERIFY(result == vk::Result::eSuccess);
        ...
        vk::Image *swapchainImages = (vk::Image *)malloc(swapchainImageCount * sizeof(vk::Image));
    ...
    void demo_prepare_cube_data_buffer()
    {
        mat4x4 VP;
    ...
    void demo_prepare_depth()
    {
        ...
        demo_set_image_layout(depth.image, vk::ImageAspectFlagBits::eDepth, vk::ImageLayout::eUndefined,
            vk::ImageLayout::eDepthStencilAttachmentOptimal, (vk::AccessFlagBits)0);
    ...
    void demo_prepare_framebuffers()
    {
        ...
        framebuffers = (vk::Framebuffer*)malloc(swapchainImageCount * sizeof(vk::Framebuffer));
    ...
    vk::ShaderModule demo_prepare_fs()
    {
        size_t size;
        void *fragShaderCode = demo_read_spv("cube-frag.spv", &size);

        frag_shader_module = demo_prepare_shader_module(fragShaderCode, size);
    ...
    void demo_prepare_pipeline()
    {
        ...
        vk::PipelineShaderStageCreateInfo const shaderStageInfo[2] =
        {
            vk::PipelineShaderStageCreateInfo()
                ...
                .setModule(demo_prepare_vs())
                .setPName("main"),
            vk::PipelineShaderStageCreateInfo()
                .setStage(vk::ShaderStageFlagBits::eFragment)
                .setModule(demo_prepare_fs())
                .setPName("main")
        };
    ...
    vk::ShaderModule demo_prepare_shader_module(const void *code, size_t size)
    {
    ...
    void demo_prepare_texture_image(const char *filename, texture_object *tex_obj, vk::ImageTiling tiling,
        vk::ImageUsageFlags usage, vk::MemoryPropertyFlags required_props)
    {
        ...
        demo_set_image_layout(tex_obj->image, vk::ImageAspectFlagBits::eColor, vk::ImageLayout::ePreinitialized, tex_obj->imageLayout,
    ...
    void demo_prepare_textures()
    {
        ...
        for(uint32_t i = 0; i < texture_count; i++)
        {
            if((props.linearTilingFeatures & vk::FormatFeatureFlagBits::eSampledImage) && !use_staging_buffer)
            {
                ...
                demo_prepare_texture_image(tex_files[i], &textures[i], vk::ImageTiling::eLinear,
            ...
            else if(props.optimalTilingFeatures & vk::FormatFeatureFlagBits::eSampledImage)
            {
                ...
                demo_prepare_texture_image(tex_files[i], &staging_texture, vk::ImageTiling::eLinear,
                    vk::ImageUsageFlagBits::eTransferSrc,
                    vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent);

                demo_prepare_texture_image(tex_files[i], &textures[i], vk::ImageTiling::eOptimal,
                    vk::ImageUsageFlagBits::eTransferDst | vk::ImageUsageFlagBits::eSampled,
                    vk::MemoryPropertyFlagBits::eDeviceLocal);

                demo_set_image_layout(staging_texture.image, vk::ImageAspectFlagBits::eColor, staging_texture.imageLayout,
                    vk::ImageLayout::eTransferSrcOptimal, vk::AccessFlags());

                demo_set_image_layout(textures[i].image, vk::ImageAspectFlagBits::eColor, textures[i].imageLayout,
                ...
                demo_set_image_layout(textures[i].image, vk::ImageAspectFlagBits::eColor, vk::ImageLayout::eTransferDstOptimal,
    ...
    vk::ShaderModule demo_prepare_vs()
    {
        size_t size;
        void* vertShaderCode = demo_read_spv("cube-vert.spv", &size);

        vert_shader_module = demo_prepare_shader_module(vertShaderCode, size);
    ...
    void demo_resize()
    {
        ...
        // Second, re-perform the demo_prepare() function, which will re-create the swapchain.
        demo_prepare();
    }

    void demo_set_image_layout(vk::Image image, vk::ImageAspectFlags aspectMask, vk::ImageLayout oldLayout,
    ...
    void demo_handle_xlib_event(const XEvent *event)
    {
        switch(event->type)
        {
        case ClientMessage:
    ...
    void demo_handle_xcb_event(const xcb_generic_event_t *event)
    {
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR pCmdLine, int nCmdShow)
{
    ...
    demo.demo_create_window();
    demo.demo_init_vk_swapchain();
...
int main(int argc, char **argv)
{
    ...
    if(demo.use_xlib)
    {
        demo.demo_create_xlib_window();
    }
    else
    {
        demo.demo_create_xcb_window();
#elif defined(VK_USE_PLATFORM_XCB_KHR)
        demo.demo_create_xcb_window();
