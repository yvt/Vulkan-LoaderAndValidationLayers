...
static bool ValidateAccessMaskPipelineStage(VkAccessFlags access_mask, VkPipelineStageFlags stage_mask) {
    ...
    int index = 0;
    bool supported = true;
    VkAccessFlags clear_mask;  // Used to clear bits as we parse access_mask
    ...
    while (access_mask) {
        ...
        supported = ((AccessMaskToPipeStage[index] & stage_mask) != 0);
        if (!supported) break;  // early out
        clear_mask <<= index;
        access_mask &= ~clear_mask;  // Mask off bit that's been checked
    }
    return supported;
}
