...
bool CheckStageMaskQueueCompatibility(layer_data *dev_data, VkCommandBuffer command_buffer, VkPipelineStageFlags stage_mask,
    ...
    static const VkPipelineStageFlagBits stage_flag_bit_array[] = {VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
    ...
    static std::unordered_map<VkPipelineStageFlagBits, VkQueueFlags> supported_pipeline_stages_table{
    ...
    for (const VkPipelineStageFlagBits item : stage_flag_bit_array) {
        if (stage_mask & item) {
            if ((supported_pipeline_stages_table[item] & queue_flags) == 0) {
                skip |=
                    log_msg(dev_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT,
                            reinterpret_cast<uint64_t &>(command_buffer), __LINE__, error_code, "DL",
                            "%s(): %s flag %s is not compatible with this queue family properties of this "
                            "command buffer. %s",
                            function, src_or_dest, string_VkPipelineStageFlagBits(item), validation_error_map[error_code]);
            }
        }
    }
    return skip;
