...
void *BuildUnwrappedUpdateTemplateBuffer(layer_data *dev_data, uint64_t descriptorUpdateTemplate, const void *pData) {
    ...
    std::vector<std::pair<size_t, uint64_t>> patch_list;
    std::vector<std::tuple<size_t, VkDebugReportObjectTypeEXT, void*>> template_entries;

    for (uint32_t i = 0; i < create_info.descriptorUpdateEntryCount; i++) {
        for (uint32_t j = 0; j < create_info.pDescriptorUpdateEntries[i].descriptorCount; j++) {
            size_t offset =
                create_info.pDescriptorUpdateEntries[i].offset + j * create_info.pDescriptorUpdateEntries[i].stride;
            char *update_entry = (char *)(pData) + offset;

            if ((create_info.pDescriptorUpdateEntries[i].descriptorType == VK_DESCRIPTOR_TYPE_SAMPLER) ||
                (create_info.pDescriptorUpdateEntries[i].descriptorType == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER) ||
                (create_info.pDescriptorUpdateEntries[i].descriptorType == VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE) ||
                (create_info.pDescriptorUpdateEntries[i].descriptorType == VK_DESCRIPTOR_TYPE_STORAGE_IMAGE) ||
                (create_info.pDescriptorUpdateEntries[i].descriptorType == VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT)) {
                ...
                wrapped_entry->sampler = reinterpret_cast<VkSampler>(dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(image_entry->sampler)]);
                wrapped_entry->imageView = reinterpret_cast<VkImageView>(dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(image_entry->imageView)]);
                template_entries.emplace_back(offset, VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT, reinterpret_cast<void *>(wrapped_entry));

            } else if ((create_info.pDescriptorUpdateEntries[i].descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER) ||
                       (create_info.pDescriptorUpdateEntries[i].descriptorType == VK_DESCRIPTOR_TYPE_STORAGE_BUFFER) ||
                       (create_info.pDescriptorUpdateEntries[i].descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC) ||
                       (create_info.pDescriptorUpdateEntries[i].descriptorType == VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC)) {
                ...
                wrapped_entry->buffer = reinterpret_cast<VkBuffer>(dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(buffer_entry->buffer)]);
                template_entries.emplace_back(offset, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT, reinterpret_cast<void *>(wrapped_entry));

            } else if ((create_info.pDescriptorUpdateEntries[i].descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER) ||
                       (create_info.pDescriptorUpdateEntries[i].descriptorType == VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER)) {
                allocation_size = std::max(allocation_size, offset + sizeof(VkBufferView));

                VkBufferView *wrapped_entry = new VkBufferView;
                *wrapped_entry = reinterpret_cast<VkBufferView>(dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(update_entry)]);
                template_entries.emplace_back(offset, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_VIEW_EXT, reinterpret_cast<void *>(wrapped_entry));
            }
        }
    }
    ...
    for (auto &this_entry : template_entries) {
        ...
        switch (std::get<1>(this_entry)) {
            case VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT: {
                //VkDescriptorImageInfo *image_dest = reinterpret_cast<VkDescriptorImageInfo *>(destination);
                //VkDescriptorImageInfo *image_source = reinterpret_cast<VkDescriptorImageInfo *>(source);
            ...
            case VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT: {
                //VkDescriptorBufferInfo *buffer_dest = reinterpret_cast<VkDescriptorBufferInfo *>(destination);
                //VkDescriptorBufferInfo *buffer_source = reinterpret_cast<VkDescriptorBufferInfo *>(source);
            ...
            case VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_VIEW_EXT: {
                //VkBufferView *buffer_view_dest = reinterpret_cast<VkBufferView *>(destination);
                //VkBufferView *buffer_view_source = reinterpret_cast<VkBufferView *>(source);
                //*buffer_view_dest = *buffer_view_source;
