...
It must also define and export these functions:
 ...
 - `<layerName>GetInstanceProcAddr(instance, pName)` either returns NULL, a local entry point or the value obtained
   ...
   - when `instance` is invalid the layer may return NULL.
   ...
   - when `pName` is `vkCreateInstance`, it returns a function pointer to the
     layer's vkCreateInstance required for chaining.
   - when `pName` is `vkCreateDevice`, it returns a function pointer to the
     layer's vkCreateDevice required for chaining.
   - when `pName` is an instance level Vulkan command that the layer intercepts,
     it returns a function pointer to the intercept routine.
   - when `pName` is an instance level Vulkan command that the layer does NOT intercept,
     it returns the value from calling the next entity's GetInstanceProcAddr.
   - when `pName` is a device level Vulkan command, the return value may be any of
     NULL, a local entry point, or the value from calling the next entity's GetInstanceProcAddr.
 ...
 - `<layerName>GetDeviceProcAddr` either returns NULL, a local entry point or the value obtained
   ...
   Detailed specification of this function,
   - when `pName` is `vkGetDeviceProcAddr`, it returns a function pointer
     to itself.
   - when `pName` is a device level Vulkan command that the layer intercepts,
     it returns a function pointer to the intercept routine.
...
any commands other than those required to be exported.
