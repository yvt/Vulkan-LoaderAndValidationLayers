...
VK_LAYER_EXPORT VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL vkGetDeviceProcAddr(VkDevice device, const char* funcName)
{
    ...
    VkLayerDispatchTable *pDisp =  device_dispatch_table(device);
    if (deviceExtMap.size() != 0 && deviceExtMap[pDisp].wsi_swapchain_enabled)
    {
...
VK_LAYER_EXPORT VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL vkGetInstanceProcAddr(VkInstance instance, const char* funcName)
{
    ...
    // EXT_debug_report
    if (inst->exts.debug_report_enabled) {
    ...
    //KHR_surface
    if (inst->exts.wsi_surface_enabled)
    {
    ...
    // KHR_XXX_surface
#ifdef VK_USE_PLATFORM_XCB_KHR
    if (inst->exts.wsi_surf_xcb_enabled)
    {
...
#endif // VK_USE_PLATFORM_XCB_KHR
#ifdef VK_USE_PLATFORM_XLIB_KHR
    if (inst->exts.wsi_surf_win32_enabled)
    {
...
#endif // VK_USE_PLATFORM_XLIB_KHR
#ifdef VK_USE_PLATFORM_MIR_KHR
    if (inst->exts.wsi_surf_mir_enabled)
    {
...
#endif // VK_USE_PLATFORM_MIR_KHR
#ifdef VK_USE_PLATFORM_WAYLAND_KHR
    if (inst->exts.wsi_surf_wayland_enabled)
    {
